...
<div style="background: linear-gradient(45deg,{tableColors.join(',')})">
  ...
  >
    <Dropdown>
      ...
      <div slot="menu">
        ...
        {#if table.type == "view"}
          ...
          {#if tableSources}
            <a
              href="/schema/{table.schemaId}/queries/{table.querieId}"
    ...
    <Dropdown>
      <button slot="toggle" class="border border-zinc-300 py-1 px-2 rounded"
...
<div class="text-zinc-800 flex items-center space-x-3 px-2 border-b border-zinc-200">
	...
	{#if table.type == 'view'}
		<div class="border-l flex items-center p-2 space-x-2 border-zinc-200">
			<div>Query</div>
			<a
				class="border rounded p-1 w-32 text-center border-zinc-300"
