...
<div
	class="overflow-scroll border {theme.darkBackgroundColor} bg-opacity-10 rounded {theme.tableBorderColor}"
>
	<div class="flex w-max items-stretch">
		...
		{#each selectedView.columns as column}
			...
			<div class="border-r w-64 {theme.tableBorderColor}">
				...
				>   
                    <span class="{column.source.table.color} leading-4 px-1 inline-block text-center rounded text-xs">
			...
			{#each records as record, j}
				<div class="flex border-b {theme.tableBorderColor}">
					{#each record as cell, i}
						<div class="w-64 border-r space-y-1 {theme.tableBorderColor} p-2 {getColumnIndex(selectedView,inspector.column) == i?theme.darkBackgroundColor:""}">
                            
							{#if selectedView.columns[i].aggregation == 'List'}
								{#if Array.isArray(cell)}
									{#each cell as item}
										<div
											class="inline-block mr-1 px-2 rounded-xl {selectedView.columns[i].source.table
												.color} bg-opacity-30"
										>
											{item}
										</div>
