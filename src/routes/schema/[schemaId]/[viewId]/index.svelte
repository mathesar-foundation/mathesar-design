<script>
	...
	async function loadData() {
		...
		let viewCount = entities.views.filter((v) => v.name.startsWith('New View')).length;
	...
	function addColumn(table, column, _column) {
		...
		if (column !== _column) {
			newColumn.alias = `${source.name}_${_column.name}_${column.name.replace('Id', '')}`;
		}
	...
	function getColumnRecords(columns) {
		let records = columns.map((c) => {
			...
			if (c.source.link.column.name == c.name) {
				if (!!getMapTable(c.source.link.table, selectedView.baseTable)) {
					let fkIdx = c.source.table.columns.indexOf(
						c.source.table.columns.find(
							(col) => col.name == getMapTable(c.source.link.table, selectedView.baseTable).column
						)
					);
				...
				columnIdx = getColumnNameIndex(c.source.link.table, c.source.link.column); //c.source.link.table.columns.indexOf(c.source.link.column)
	...
	function applySteps(records, steps) {
		...
		Object.keys(selectedView.steps).forEach((step) => {
			...
			} else if (selectedView.steps[step].type == 'summarize' && !selectedView.steps[step].hidden) {
	...
	function updateAggregation(aggregation, column, columnIdx) {
		...
		//newColumn.source.table.columns.find((c) => c.name == newColumn.name).type = newColumn.type;
...
{:then entities}
	...
	>
		...
		<div class="w-7/12 flex-grow p-4">
			...
			>
				...
				{:else if !selectedView.baseTable}
					<div
						class="border rounded text-center {theme.mutedTextColor} {theme.tableBorderColor} p-10 flex-grow"
					>
						Select Base Table
					</div>
				{:else}
		...
		<div class="flex flex-col h-full overflow-y-scroll w-80 {theme.darkBackgroundColor}">
			...
			{#if inspector.action == 'Query Details'}
				<div class="p-2 space-y-2">
					...
					<p class="text-xs {theme.mutedTextColor}">
						This view is a virtual table based on a SQL statement's result set. The SQL below can be
						used to recreate this view.
					</p>
			...
			{#if inspector.action == 'Column'}
				<ColumnEditor
					...
					on:updateAggregation={(e) =>
						updateAggregation(e.detail.aggregation, e.detail.column, e.detail.columnIdx)}
