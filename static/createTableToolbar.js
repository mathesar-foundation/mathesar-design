...
function linkTableWizard(obj) {
    ...
    referencedTable.addEventListener('change', function () {
        ...
        let questionsList = [
            `<div>Can a single <span class="$bg-indigo-500 bg-opacity-20 rounded px-1">${obj.name}</span> record be linked to more than one <span class="px-1 $bg-indigo-500 bg-opacity-20 rounded">${_table}</span> record?</div>`,
            `<div>Can a single <span class="$bg-indigo-500 bg-opacity-20 rounded px-1">${_table}</span> record be linked to more than one <span class="px-1 $bg-indigo-500 bg-opacity-20 rounded">${obj.name}</span> record?</div>`
        ];
        ...
        questionsWrapper.addEventListener('change', function () {
            ...
            if (answers.length > 1) {
                ...
                if (answers.every(answer => answer == 'yes')) {
                    ...
                    <p class="$text-zinc-500">We will create a new table to map records from <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${obj.name}</span> to records from <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${_table}</span></p>
                    <div class="my-1">
                    ...
                    if (answers[0] == 'yes') {
                        ...
                        summary.innerHTML = `
                    <h4>Under the hood</h4>
                    <p class="$text-zinc-500">We will create a new column in the <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${_table}</span> table and set up a foreign key constraint to the <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${_table}</span> table.</p>
                    <div class="my-1">
                    ...
                    `;
                        ...
                        summary.innerHTML = `
                    <h4>Under the hood</h4>
                    <p class="$text-zinc-500">We will create a new column in the <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${obj.name}</span> table and set up a foreign key constraint to the <span class="$bg-indigo-500 $text-zinc-800 bg-opacity-20 rounded px-1">${obj.name}</span> table.</p>
                    <div class="my-1">
                    <label class="block">New Column Name</label>
                    ...
                    `;
                        ...
                        summary.innerHTML = `
                    <h4>Under the hood</h4>
                    <p class="text-sm">We will create a new column named <span class="$bg-indigo-500 bg-opacity-20 rounded px-1">${_table}Id</span> in the <span class="$bg-indigo-500 bg-opacity-20 rounded px-1">${obj.name}</span> table and set up a unique and foreign key constraint to the <span class="$bg-indigo-500 bg-opacity-20 rounded px-1">${obj.name}</span> table.</p>
                    `;
